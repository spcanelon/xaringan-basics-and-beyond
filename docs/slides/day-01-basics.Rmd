...
---
name: placement
## Layouts <i class="fas fa-th-large" style = "float: right"></i>
...
.left-column[
**`left-column[]` places content into a column 20% wid**
...
.small[To insert an avatar: `<img style="border-radius: 50%;" src="https://insertweblink.here" width="170px"/>`]
]
...
You can use the [background-image property](https://github.com/gnab/remark/wiki/Markdown#background-image) just like classes

.pull-left[
- `background-image: url(https://some.pretty.picture)`
- `background-size:`
...
.pull-right[
Image from a url:<br>`url(https://unsplash.com/photos/ihDcta7Kvqg)`

Local image, use relative file paths: `url(img/camera-green.jpg)`

.my-coral[now, let's see `background-size` examples in action! `r icon::fa("arrow-circle-right")` ]
]
...
.pull-right-wide[
Use Hadley's [emo(ji) `r emo::ji("package")`](https://github.com/hadley/emo#emoji):<br>
...
Or use the [icon `r emo::ji("package")`](https://slides.mitchelloharawild.com/icon/): ` r icon::fa("brain")`<br>
]
...
.footnote[Make sure to check out the GT package workshop tomorrow!]
---
name: plots
...
### Great tips here from Alison Hill

```{r}
knitr::include_url("https://rstudio-education.github.io/teaching-in-production/slides/index.html#46")
```
...
.right-column[
### Make a new slide and practice adding some R content

----

Think about adding a table or a plot
